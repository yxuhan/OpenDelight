...
def load_part_mae_model(ckpt_path, model):
    ...
    for k in load_state_dict.keys():
        if k not in checkpoint['model'].keys():
            print("[Warning] Key {} not found in checkpoint.".format(k))
            continue
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, device, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, num_reg_tokens=1,
        ...
        self.reg_token = nn.Parameter(torch.zeros(2, num_reg_tokens, embed_dim))
        nn.init.normal_(self.reg_token, std=1e-6)  # Initialize parameters with small values (similar to VGGT)
    ...
    def forward(self, x, data_source):
        x = (x - self.imagenet_mean) / self.imagenet_std
        ...
        cls_token = self.cls_token + self.pos_embed[:, :1, :]
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, self.reg_token[data_source], x), dim=1)
...
def get_mae_encoder(img_size, model_type, device, num_reg_tokens, **kwargs):
    if model_type == "base":
        model = mae_vit_base_patch16(img_size=img_size, device=device, num_reg_tokens=num_reg_tokens)
        # model_path = 'pretrained/mae/mae_pretrain_vit_base.pth'
        model_path = 'pretrained/mae/mae_visualize_vit_base.pth'
    elif model_type == "large":
        model = mae_vit_large_patch16(img_size=img_size, device=device, num_reg_tokens=num_reg_tokens)
...
if __name__ == "__main__":
    model = get_mae_encoder(img_size=224, model_type="base", device="cpu", num_reg_tokens=1)
